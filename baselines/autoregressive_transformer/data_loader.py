...
class JSONDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target = [major_or_minor] + [key_signature] + ["SEP"] + target + ["EOS"]
        # Tokenize the target
        target = [self.tokenizer[note] for note in target if note in self.tokenizer]

        # Pad the target to the maximum sequence length
        ...
        target_attention_mask = torch.where(target != 0, 1, 0)
        target_attention_mask = target_attention_mask.float()
        
        train_data = {"input_ids": target, "labels": target, "attention_mask": target_attention_mask}
